...
async def get_deployment_metrics(
    ...
    try:
        ...
        deployment = active_deployments[deployment_id]
        namespace = deployment.get("namespace")
        release_name = deployment.get(
            "release_name"
        )  # Changed from "name" to "release_name"

        # Add debug logging to help diagnose issues
        logger.info(
            f"Fetching metrics for deployment {deployment_id}: namespace={namespace}, release_name={release_name}"
        )
        logger.debug(f"Full deployment info: {deployment}")

        if not namespace or not release_name:
            return MetricsResponse(
                success=False,
                message=f"Deployment information incomplete: namespace={namespace}, release_name={release_name}",
            )
